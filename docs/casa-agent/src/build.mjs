// Inline the two screenshots so the use-case document opens offline.
// Run after build-derivatives.sh, from any directory.
...
const html = readFileSync(resolve(src, 'use-cases.template.html'), 'utf8')
  .replace(/\{\{IMG:([a-z-]+)\}\}/g, (_, name) => {
    images++;
    return 'data:image/webp;base64,' + readFileSync(resolve(root, `images/full/casa-agent/${name}.webp`)).toString('base64');
  });
...
writeFileSync(resolve(root, 'portfolio/casa-agent/use-cases.html'), html);
