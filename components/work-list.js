...
export class WorkList extends LitElement {
  ...
  static meta = {
    ...
    'Destination Home': {
      no: '02', status: 'client', statusLabel: 'Adopted by Sterling Lexicon',
      role: 'Solo — product, engineering, consulting',
      summary: 'A chat-first relocation assistant for corporate transferees, built for and adopted by Sterling Lexicon. The original brief centered on OpenAI with web search. I proposed extending that approach with structured data sources for local amenities, rents and other facts that shape a relocation decision, then implemented those additions. Intake happens as a conversation, program questions get answered only where a cited passage supports them, and neighborhoods are ranked by deterministic code over sourced measurements rather than by the model, which is what makes the ranking auditable under fair-housing scrutiny. The hard part was not the chat. It was deciding what the system is allowed to claim: absent data stays null with a coverage figure instead of becoming a neutral score, and a factor that cannot be measured well enough simply does not publish. Screenshots show the earlier development build with placeholder program content and seeded measurement data.',
    },
    'Casa Agent': {
